# etl/extract/read_gold.py
# ============================================================
# Centralized gold layer reader for all ETL pipeline scripts.
...
# ------------------------------------------------------------
# gold layer root — single source of truth for all paths.
# Change this one constant if the data lake location ever moves.
# ------------------------------------------------------------
GOLD_ROOT = Path(__file__).resolve().parents[2] / "data_lake" / "curated"
...
def read_gold(
    ...
    if execution_date is None:
        execution_date = get_last_gold_date()

    filepath = GOLD_ROOT / execution_date / f"{filename}.parquet"

    # ----------------------------------------------------------
    # Guard: file must exist before we attempt to read it
    # ----------------------------------------------------------
    if not filepath.exists():
        logger.error(f"Gold file not found: {filepath}")
        raise FileNotFoundError(f"Gold file not found: {filepath}")

    logger.info(f"Reading Gold file: {filepath}")
    ...
    # ----------------------------------------------------------
    logger.info(
        f"Gold read complete | file={filename}.parquet | "
