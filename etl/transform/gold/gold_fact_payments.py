...
def run():
    with PipelineAuditor(
        ...
        # Read Silver Parquet
        try:
            df, execution_date = read_silver("fact_payments")
            if df.empty:
                logger.info("No payments records for this date — skipping")
                return
        except FileNotFoundError:
            logger.info("fact_payments.parquet not found for this date — skipping")
            return
        rows_read = len(df)
        logger.info(f"Rows read from Silver: {rows_read}")
        OUTPUT_PATH = PROJECT_ROOT / "data_lake" / "curated" / execution_date / "fact_payments.parquet"

        # Read exchange rates
        rates_df = pd.read_csv(RATES_PATH)

        rates_df = rates_df.rename(columns={"date_sk": "payment_date_sk"})
        df = df.merge(
            rates_df[["payment_date_sk", "currency_code", "rate_to_inr"]],
            on=["payment_date_sk", "currency_code"],
            how="left"
        )
        ...
        if missing_rate_mask.sum() > 0:
            for _, row in df[missing_rate_mask].iterrows():
                auditor.log_rejected_record(
                    ...
                    rejection_reason=f"missing exchange rate for {row['currency_code']} on {row['payment_date_sk']}",
                    raw_data=row.to_dict()
                )

        # NaN * anything = NaN — so _inr columns are automatically NULL for missing rates
        df["amount_inr"]   = (df["amount"] * df["rate_to_inr"]).round(2)
